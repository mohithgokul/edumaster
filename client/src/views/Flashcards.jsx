...
export default function Flashcards() {
  ...
  const [expandedSummarySubjects, setExpandedSummarySubjects] = useState(new Set());
  ...
  useEffect(() => {
    const fetchCards = async () => {
      try {
        ...
        // Fetch generated summaries per subject for subject picker quick preview.
        try {
          const materialsRes = await api.get('/materials');
          const summaryMap = {};

          materialsRes.data.forEach((mat) => {
            const summaryText = mat.summary?.trim();
            if (!summaryText) return;

            if (!summaryMap[mat.subject]) summaryMap[mat.subject] = [];
            summaryMap[mat.subject].push({
              id: mat._id,
              title: mat.title,
              summary: summaryText,
            });
          ...
          setSubjectSummaries(summaryMap);
        } catch {
          setSubjectSummaries({});
        }
      } catch {
  ...
  const toggleSubjectSummary = (subjectName) => {
    setExpandedSummarySubjects(prev => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {subjects.map(sub => (
            <div
              ...
              style={{ border: '1px solid var(--border)', transition: 'all 0.2s', background: 'none', width: '100%' }}
            ...
            >
              ...
              {!!subjectSummaries[sub.name]?.length && (
                <div style={{ marginTop: '12px' }}>
                  <button
                    onClick={() => toggleSubjectSummary(sub.name)}
                    style={{
                  ...
                  >
                    {expandedSummarySubjects.has(sub.name) ? 'Hide Summary' : 'View Summary'}
                  </button>

                  {expandedSummarySubjects.has(sub.name) && (
                    <div
                      style={{
                        marginTop: '10px',
                        border: '1px solid var(--border)',
                    ...
                    >
                      {subjectSummaries[sub.name].map(item => (
                        <div key={item.id} style={{ marginBottom: '10px' }}>
                          <p style={{ color: 'var(--text-primary)', fontWeight: 700, marginBottom: '4px' }}>{item.title}</p>
